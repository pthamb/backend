...
def register(request):
    ...
    # Check the content type of the uploaded file
        if picture.content_type.startswith("image"):
            # It's an image file, so process it as an image
            try:
                img = Image.open(picture)
                img.thumbnail((800, 800))  # Resize the image as needed

                # Save the resized image to a BytesIO object
                buffered = BytesIO()
                img_format = img.format  # Get the image format (e.g., JPEG, PNG)
                img.save(buffered, format=img_format)
                image_data = b64encode(buffered.getvalue()).decode()

                # Prepend the data URI scheme and base64 marker
                picture_data = f"data:image/{img_format.lower()};base64,{image_data}"
            except Exception as e:
                # Handle exceptions
                print(f"Error handling image: {e}")
        else:
            # It's not an image; assume it's a URL and create a dictionary with the URL
            picture_data = {"url": picture.read().decode()}    
        ...
        confirm_url = f'https://neighborgood.io/verification/{user_profile.confirmation_token}'
...
def google_login(request):
    ...
    try:
        ...
        if created:
            ...
            user_profile = UserProfile.objects.create(user=user, name=name, email=email, picture=picture, email_confirmed=True)
...
def calculate_similarity(user1, user2):
    fields = [
        'walking', 'running', 'dog', 'gardening', 'swimming', 
        'coffeeTea', 'art', 'foodGathering', 'sports', 'movies', 
        'shopping', 'happyHours', 'rides', 'childcare', 'eldercare', 
        'petcare', 'tutoring', 'repairAdvice'
    ]
    matches = 0
    valid_fields = 0
    matching_fields = []
    ...
    for field in fields:
        ...
        if user1_interested or user2_interested:
            valid_fields += 1

            # Count as a match only if both are interested
...
def find_similar_users(request):
    try:
        ...
        similar_users = []
        user_profiles = UserProfile.objects.exclude(user=current_user).exclude(latitude__isnull=True).exclude(longitude__isnull=True)
        for user in  user_profiles:
            ...
            if distance <= 15 and similarity> 50:
                    similar_users.append(user_profile_to_dict(user))
